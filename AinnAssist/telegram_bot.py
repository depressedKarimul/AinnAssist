...
async def set_webhook(update: Update, context: ContextTypes.DEFAULT_TYPE):
    webhook_url = f"{os.getenv('API_URL').rstrip('/ask')}/telegram-webhook"
    await context.bot.setWebhook(webhook_url)
    await update.message.reply_text("Webhook set!")
...
def main():
    ...
    print("🤖 Bot is ready for webhook...")
    app.run_polling()  # For local testing; Railway uses webhook